...
class LabelViewsTests(TestCase):
    ...
    def test_label_create_view(self):
        ...
        self.assertIn('Метка успешно создана.', [m.message for m in messages_list])
    ...
    def test_label_update_view(self):
        ...
        messages_list = list(get_messages(response.wsgi_request))
        self.assertIn('Метка успешно изменена', [m.message for m in messages_list])
    ...
    def test_label_delete_view(self):
        ...
        messages_list = list(get_messages(response.wsgi_request))
        self.assertIn('Метка успешно удалена', [m.message for m in messages_list])
    ...
    def test_label_delete_view_with_tasks(self):
        ...
        task = Task.objects.create(title='Test Task', author=self.user, status=status)
        ...
        messages_list = list(get_messages(response.wsgi_request))
        self.assertIn('Невозможно удалить метку, т.к. она связана с задачей.', [m.message for m in messages_list])